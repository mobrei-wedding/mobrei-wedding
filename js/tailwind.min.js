$(document).ready(function(){renderBill()}),$("#download-report").click(function(){downloadPdf(pdfDocument)}),$("#test-report").click(function(){var e=generatePdfContext();pdfDocument=buildPdf(e),openPdf(pdfDocument)});function sendPdfToEmail(e){var t=pdfMake.createPdf(pdfDocument),i=t.getBase64(()=>{});i.then(function(t){sendEmail(t,e)})}function returnSendPdfToEmailPromise(e){var t=pdfMake.createPdf(pdfDocument),i=t.getBase64(()=>{});return i.then(function(t){console.log("Attachment"),sendEmail(t,e)})}function updateBillVars(e,t,i){var n=document.getElementById("ff-id-2046362136"),a=document.getElementById("Display2046362136"),l=document.getElementById("Display2139224654"),o=document.getElementById("Display1588677463");a.value=e,l.value=t,o.value=i,n.value=e}function calculateProduct(){var e=0,t=0,i=document.getElementById("ff-id-2046362136"),n=document.getElementById("Display2046362136"),a=document.getElementById("Display2139224654"),l=document.getElementById("Display1588677463"),o=document.getElementById("ff-id-1959728480");for(var r in product_data){var d=product_data[r];d.quantity&&(e+=d.quantity*d.price,t+=d.quantity)}n.value=e,l.value=e+1125,i.value=e,a.value=t}function getDocument(){return document}function renderBill(){var e=document.getElementById("bill-table-1959728480");e.body=null,header=` <colgroup>
                <col>
                <col>
                <col>
                <col>
            </colgroup>
            <thead>
                <tr>
                <td>項目</td>
                <td>單價</td>
                <td>數量</td>
                <td>總價</td>
                </tr>
            </thead><tbody>`;var t=[];for(var i of shoppingCart){var n=product_data[i],a=n.quantity*n.price,l="<tr><td>"+n.title+"</td><td>"+n.price+"</td><td>"+n.quantity+"</td><td>"+a+"</td></tr>";t.push(l)}var o=document.getElementById("ff-id-2046362136").value,r=1125,d=(o?o:0)+r,s=`<tfoot>
        <tr>
            <td colspan="3">周邊總價</td>
            <td>NT$${o?o:0}</td>
        </tr>
        <tr>
            <td colspan="3">茶會報名費</td>
            <td>NT$${r}</td>
        </tr>
        <tr>
            <td colspan="3">最終價錢</td>
            <td>NT$${d}</td>
        </tr> </tfoot>`;e.innerHTML=header+t.join(" ")+"</tbody>"+s}function getNextSectionId(e){var t=getSectionsItems(),i=t.findIndex(t=>t.id==e),n=t[i+1];return n?n.id:e}function getPreviousSectionId(e){var t=getSectionsItems(),i=t.findIndex(t=>t.id==e),n=t[i-1];return n?n.id:e}function jumptoSection(e,t,i,n){var a=getSectionsItems();a.forEach(e=>{var t="ff-sec-"+e.id,i=document.getElementById(t);i.style.display=e.id===n?"block":"none"})}function endSection(){var e=getSectionsItems();e.forEach(e=>{var t="ff-sec-"+e.id,i=document.getElementById(t);"ending"===e.id?i.style.display="block":$(`#${t}`).remove()})}function pdfHeader(e){return{alignment:"center",text:`MobReiWedding#${e} Details`,style:"header",fontSize:23,bold:!0,margin:[0,10]}}function getToday(){var e=new Date;const t=e.toLocaleDateString("zh-Hans-CN",{timeZone:"Asia/Taipei"});return t}function getBase64ImageFromURL(e){return new Promise((t,i)=>{var n=new Image;n.setAttribute("crossOrigin","anonymous"),n.onload=()=>{var e=document.createElement("canvas");e.width=n.width,e.height=n.height;var i=e.getContext("2d");i.drawImage(n,0,0);var a=e.toDataURL("image/png");t(a)},n.onerror=e=>{i(e)},n.src=e})}function generateUlItem(e,t){return{italics:!1,ul:[{text:e+"\n"},[{text:t+"\n"}]],text:[{text:t+"\n"}]}}function generateSection(e){return{italics:!1,text:[{text:e+"\n",style:"invoiceBillingTitle"}]}}function getBillData(){var e=[],t=0;for(var i in product_data){var n=product_data[i];if(n.quantity){var a=n.quantity*n.price;t+=a,e.push({項目:n.title,單價:n.price,數量:n.quantity,總價:a})}}return{billData:e,total:t}}function openPdf(e){pdfMake.createPdf(e).open({},window)}function downloadPdf(e){pdfMake.createPdf(e).download("report.pdf")}function buildPdf(e){pdfMake.fonts={Roboto:{normal:"Roboto-Regular.ttf",bold:"Roboto-Medium.ttf",italics:"Roboto-Italic.ttf",bolditalics:"Roboto-Italic.ttf"},AaGuDianKeBenSong:{normal:"AaGuDianKeBenSong.ttf",bold:"AaGuDianKeBenSong.ttf",italics:"AaGuDianKeBenSong.ttf",bolditalics:"AaGuDianKeBenSong.ttf"},characters:{normal:"characters.ttf",bold:"characters.ttf",italics:"characters.ttf",bolditalics:"characters.ttf"}};var t={info:{title:"MobReiWedding Ref#"+e.id,author:"One And Only One",subject:"MobRei wedding Register Form",keywords:"MobRei Wedding"},content:e.context,defaultStyle:{font:"characters",fontSize:11,color:"#595553",lineHeight:1.2,columnGap:20},styles:{header:{fontSize:18,bold:!0},itemTitle:{fontSize:12},itemContext:{color:"#595553"},p:{marginTop:11},ul:{marginTop:11},ol:{marginTop:11},h1:{marginTop:36,fontSize:36},h2:{fontSize:24,marginTop:10},h3:{fontSize:20,bold:!0,italics:!0,marginTop:10},h4:{fontSize:12,bold:!0,marginTop:10},documentHeaderLeft:{fontSize:10,margin:[5,5,5,5],alignment:"left",bold:!0},documentHeaderCenter:{fontSize:10,margin:[5,5,5,5],alignment:"center",bold:!0},documentHeaderRight:{fontSize:10,margin:[5,5,5,5],alignment:"right",bold:!0},documentFooterLeft:{fontSize:10,margin:[5,5,5,5],alignment:"left"},documentFooterCenter:{fontSize:10,margin:[5,5,5,5],alignment:"center"},documentFooterRight:{fontSize:10,margin:[5,5,5,5],alignment:"right"},invoiceTitle:{fontSize:22,bold:!0,alignment:"right",margin:[0,0,0,15],bold:!0},invoiceSubTitle:{fontSize:10,alignment:"right",color:"#aaacaa"},invoiceSubValue:{fontSize:10,alignment:"right",color:"#aaacaa"},invoiceBillingTitle:{fontSize:12,bold:!0,alignment:"left",margin:[0,20,0,5],bold:!0},invoiceBillingDetails:{alignment:"left",fontSize:10,color:"gray"},invoiceBillingAddressTitle:{margin:[0,7,0,3],fontSize:12,bold:!0},invoiceBillingAddress:{fontSize:10,color:"gray"},itemsHeader:{margin:[0,5,0,5],bold:!0,fontSize:12},itemTitle:{bold:!0,fontSize:11},itemSubTitle:{italics:!0,fontSize:10},itemNumber:{margin:[0,5,0,5],alignment:"center"},itemTotal:{margin:[0,5,0,5],bold:!0,alignment:"center"},itemsFooterSubTitle:{margin:[0,5,0,5],bold:!0,fontSize:10,alignment:"right"},itemsFooterSubValue:{margin:[0,5,0,5],bold:!0,fontSize:10,alignment:"center"},itemsFooterTotalTitle:{margin:[0,5,0,5],bold:!0,fontSize:10,alignment:"right"},itemsFooterTotalValue:{margin:[0,5,0,5],bold:!0,fontSize:10,alignment:"center"},signaturePlaceholder:{margin:[0,70,0,0]},signatureName:{bold:!0,alignment:"center"},signatureJobTitle:{italics:!0,fontSize:10,alignment:"center"},notesTitle:{fontSize:10,bold:!0,margin:[0,50,0,3]},notesText:{fontSize:10},center:{alignment:"center"}}};return t}function generatePdfContext(){var e=document.getElementById("Widget658159440").value,t=document.getElementById("Widget1439048663").value,i=document.getElementById("Widget981692748").value,n=document.getElementById("Widget111610196").value,a=document.getElementById("Widget896231682").value,l=generatePdfBillHeader(n,a),o=getBillData(),r=buildPdfTable(o,bill_col_data),d=buildPdfTableFooter(o.total,1125,o.total+1125),s=buildPdfNote(),u=[generatePdfTitle(),generatePdfHeader(e,t,i),...l,"\n\n",generateSection("\u8336\u6703\u5831\u540D\u660E\u7D30\u5982\u4E0B\uFF1A",""),r,d,...s];return{context:u,id:i}}function returnLogoDabaseUri(){return`data:image/png;base64,`}function returnTitleStandardBlonde(){return`data:image/png;base64,`}function generatePdfTitle(){return{image:returnTitleStandardBlonde(),width:400,style:"documentHeaderCenter"}}function generatePdfHeader(e,t,i){return{columns:[[{stack:[{columns:[{text:"",style:"invoiceSubTitle",width:"*"},{text:"ref. #"+i,style:"invoiceSubValue",width:120}]},{columns:[{text:"e-mail ",style:"invoiceSubTitle",width:"*"},{text:e,style:"invoiceSubValue",width:120}]},{columns:[{text:"\u586B\u5BEB\u65E5\u671F",style:"invoiceSubTitle",width:"*"},{text:getToday(),style:"invoiceSubValue",width:120}]},{columns:[{text:"\u586B\u5BEB\u4EBA\u66B1\u7A31",style:"invoiceSubTitle",width:"*"},{text:t,style:"invoiceSubValue",width:120}]}]}]]}}function generatePdfSecondPageHeader(e,t){return{columns:[{image:returnLogoDabaseUri(),width:150},[{text:"\u554F\u5377\u5167\u5BB9",style:"invoiceTitle",width:"*"},{stack:[{columns:[{text:"e-mail #",style:"invoiceSubTitle",width:"*",color:"gray"},{text:e,style:"invoiceSubValue",width:100,color:"gray"}]},{columns:[{text:"\u586B\u5BEB\u65E5\u671F",style:"invoiceSubTitle",width:"*",color:"gray"},{text:getToday(),style:"invoiceSubValue",width:100,color:"gray"}]},{columns:[{text:"\u586B\u5BEB\u4EBA\u66B1\u7A31",style:"invoiceSubTitle",width:"*",color:"gray"},{text:t,style:"invoiceSubValue",width:100,color:"gray"}]}]}]]}}function generatePdfSentence(e,t){return{italics:!1,text:[{text:" - "+e+"\uFF1A\n",style:"itemTitle",bold:!0},{text:"   "+t+"\n",style:"itemContext",bold:!1},"\n"]}}function generatePdfItem(e,t){return{italics:!1,ul:[{text:e+"\uFF1A\t"+t,style:["itemTitle"]}]}}function generatePdfBillHeader(e,t){return[{columns:[{text:"\u4F86\u8CD3\u59D3\u540D",style:"invoiceBillingTitle"}]},{columns:[{text:e+"\n",style:"invoiceBillingDetails"}]},{columns:[{text:"\u4F86\u8CD3\u6536\u4EF6\u5730\u5740",style:"invoiceBillingAddressTitle"}]},{columns:[{text:t,style:"invoiceBillingAddress"}]}]}function buildPdfTableBody(e,t){var i=[],n=t.map(e=>({borderColor:["#ffffff","#ffffff","#ffffff","#bfbfbf"],text:e}));i.push(n);var a=e.billData,l=e.total;a.forEach(function(e){var n=[];t.forEach(function(t){var i={borderColor:["#ffffff","#ffffff","#ffffff","#bfbfbf"],fontSize:9,text:e[t]};n.push(i)}),i.push(n)});return i.push([{text:"",border:[!1,!1,!1,!1]},{text:"",border:[!1,!1,!1,!1]},{text:"",border:[!1,!1,!1,!1]},{text:"",border:[!1,!1,!1,!1]}]),i}function buildPdfTable(e,t){return{style:"tableStyle",layout:{},table:{widths:["25%","25%","25%","25%"],heights:20,headerRows:1,body:buildPdfTableBody(e,t)}}}function buildPdfTableFooter(e,t,i){return{table:{headerRows:0,widths:["*",80],borderColor:["#ffffff","#ffffff","#ffffff","#ffffff"],body:[[{text:"\u5468\u908A\u7E3D\u50F9",style:"itemsFooterSubTitle"},{text:"NT$ "+e,style:"itemsFooterSubValue"}],[{text:"\u5A5A\u79AE\u5831\u540D\u8CBB",style:"itemsFooterSubTitle"},{text:"NT$ "+t,style:"itemsFooterSubValue"}],[{text:"= \u7E3D\u8CBB\u7528",style:"itemsFooterTotalTitle"},{text:"NT$ "+i,style:"itemsFooterTotalValue"}]]},layout:"noBorders"}}function buildPdfNote(){return[{text:"NOTES",style:"notesTitle",decoration:"underline",decorationColor:"red",color:"red"},{text:"\u6B64\u8868\u55AE\u50C5\u4F5C\u70BA\u586B\u5BEB\u8B49\u660E\uFF0C\u8ACB\u6CE8\u610F\u5F8C\u7E8C\u6709\u7121\u6536\u5230\u5831\u540D\u532F\u6B3E\u901A\u77E5",style:"notesText",decoration:"underline",decorationColor:"red",color:"red"}]}function getSectionsItems(){return[{id:"root",items:[{id:"1439048663",type:"",entryType:"text",entryId:"1068526554"},{id:"658159440",entryType:"text",entryId:"883070371",regex:"^(.+)@(.+)$",errorMsg:"\u8ACB\u518D\u78BA\u8A8D\u4E00\u6B21\u4FE1\u7BB1\u683C\u5F0F\uFF5E"},{id:"981692748",entryType:"text",entryId:"1053365713",regex:"^[0-9]{3}$",errorMsg:"\u683C\u5F0F\u70BA\u4E09\u4F4D\u534A\u5F62\u6578\u5B57\u5594\uFF5E"},{id:"1686927898",entryType:"text",entryId:"896231682"},{id:"1691582630",entryType:"text",entryId:"111610196"},{id:"1043102370",type:"PARAGRAPH_TEXT",entryType:"text",entryId:"1395581871"},{id:"1827994924",type:"PARAGRAPH_TEXT",entryType:"text",entryId:"1970291756"},{id:"929329530",entryType:"radio",entryId:"1841292804"},{id:"36699560",entryType:"radio",entryId:"1339607071"},{id:"895429393",entryType:"radio",entryId:"451787920"},{id:"1624480621",entryType:""},{id:"1406572988",entryType:"custom"},{id:"259046852",entryType:"custom"},{id:"304960959",entryType:"custom"},{id:"2051996369",entryType:"custom"},{id:"1706597056",entryType:"custom"},{id:"901430140",entryType:"custom"},{id:"985712023",entryType:""},{id:"1121831062",entryType:"custom"},{id:"2046362136",entryType:"bill-display",entryId:"575052231"},{id:"2139224654",entryType:"bill-display",entryId:"253014823"},{id:"855092565",entryType:"",entryId:"1423911485"},{id:"1588677463",entryType:"bill-display",entryId:"1725694623"},{id:"1959728480",entryType:""}]},{id:"1194002809",items:[{id:"218713257",entryType:"radio",entryId:"18356239"},{id:"964519823",entryType:"radio",entryId:"1286001326"},{id:"1226046570",entryType:"text",entryId:"1032881495"},{id:"368266770",entryType:"radio",entryId:"746776974"},{id:"957187906",entryType:"radio",entryId:"450550785"},{id:"1565424873",entryType:"radio",entryId:"63787071"},{id:"1666827740",entryType:"radio",entryId:"1160286542"},{id:"780521456",entryType:"text",entryId:"189841679"},{id:"1388002876",entryType:"text",entryId:"653167275"},{id:"1496947901",entryType:"text",entryId:"713268329"}]},{id:"ending",items:[]}]}function getData(){var e=getSectionsItems();return dataSet={},e.forEach(e=>{var t=e.items;t.forEach(e=>{var t="entry."+e.entryId,i="";if("radio"==e.entryType){var n=function(){var e;return $(`[name="${t}"]`).each(function(){!0===$(this).prop("checked")&&(e=$(this).val(),"__other_option__"===e&&(dataSet[`${t}.other_option_response`]=$(`input[name="${t}.other_option_response"]`).val()))}),e};i=n()}else"custom"==e.entryType?(i=product_data[e.id].quantity,t=product_data[e.id].quantity_id):i="bill-display"==e.entryType?$(`#Display${e.id}`).val()||0:$(`[name="${t}"]`).val()||"";dataSet[t]=i})}),dataSet.pageHistory="0,1",dataSet}function getUserEmail(){var e=document.getElementById("Widget658159440").value;return e}async function useFetch(e){var t=new URLSearchParams(Object.entries(e)).toString();const i=await fetch("https://docs.google.com/forms/u/8/d/e/1FAIpQLSdfsLWjzLUKRZRxsUbiJNuverhidV76_VuR3GK2YFr_pkxiNw/formResponse",{method:"POST",mode:"no-cors",cache:"no-cache",headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*",Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","Accept-Encoding":"gzip, deflate, br","Content-Type":"application/x-www-form-urlencoded"},Referer:"https://docs.google.com/forms/d/e/1FAIpQLSdfsLWjzLUKRZRxsUbiJNuverhidV76_VuR3GK2YFr_pkxiNw/formResponse",body:t});return i}function connectGoogleForm(e){return $.ajax({type:"POST",url:"https://docs.google.com/forms/u/8/d/e/1FAIpQLSdfsLWjzLUKRZRxsUbiJNuverhidV76_VuR3GK2YFr_pkxiNw/formResponse",data:e,contentType:"application/json",jsonpCallback:"processJSONPResponse",dataType:"jsonp",cache:!1,error:function(e){200==e.status?(console.log("v2.3 Submit Data!"),alert("submit succeed!")):(console.log("not 200:",e),alert("error:",e))},complete:function(){endSection()}})}function consoleAlertSuccess(){console.log("\u8CC7\u6599\u6210\u529F\u9001\u51FA\uFF01"),alert("\u8CC7\u6599\u6210\u529F\u9001\u51FA")}$("#ff-submit-1194002809").bind("click",function(){console.log("disable button"),$("#ff-submit-1194002809").prop("disabled",!0),setTimeout(function(){$("#ff-submit-1194002809").prop("disabled",!1)},1e4)});function submitForm(e,t){var i="-3"==t?0:validate(e,t);if(!(0<i)&&!this.submitting){var n=generatePdfContext();pdfDocument=buildPdf(n);var a=getData(),l=getUserEmail();useFetch(a).then(()=>{console.log("Submit Successfully!"),endSection();var e=pdfMake.createPdf(pdfDocument),t=e.getBase64(()=>{});t.then(function(e){asyncSendEmail(e,l).then(e=>{console.log("Send SMTP Email:",e),consoleAlertSuccess()})})})}}function scrollIntoView(e){e||(e=this.getContentElement()||{}),e.scrollIntoView&&e.scrollIntoView(!0)}function validate(e,t){var i=this,n=[],a=getDocument(),l=new FormData(e),o=getSectionsItems(),r=o[0];o.forEach(function(e){e.id==t&&(r=e)}),a.querySelectorAll("#ff-sec-"+r.id+" .ff-widget-error").forEach(function(e){e.style.display="none"});var d=a.getElementById("WidgetemailAddress");if(d&&!1==d.checkValidity()){var s=a.getElementById("ErroremailAddress");s.innerHTML=d.value?"<b>!</b>"+i.lang("Must be a valid email address"):"<b>!</b>"+i.lang("This is a required question"),s.style.display="block",n.push(d)}return r.items.forEach(function(e){var t=a.querySelector("#ff-id-"+e.id+" input");"PARAGRAPH_TEXT"==e.type?t=a.querySelector("#ff-id-"+e.id+" textarea"):"LIST"==e.type&&(t=a.querySelector("#ff-id-"+e.id+" select"),!t&&(t=a.querySelector("#ff-id-"+e.id+" input")));var o,r=function(i){n.push(t);var l=a.getElementById("Error"+e.id);l&&(l.innerHTML="<b>!</b>"+i,l.style.display="block")},d=!0;if(t&&(t.readOnly?(t.readOnly=!1,d=t.checkValidity(),t.readOnly=!0):d=t.checkValidity(),o=l.get(t.name),o&&e.regex)){var s=new RegExp(`${e.regex}`);d=s.test(o)}if(!1==d)t.hasAttribute("required")&&!o?r("\u9019\u662F\u5FC5\u586B\u554F\u984C\u5594\uFF01"):o?e.errorMsg?r(e.errorMsg):r("\u683C\u5F0F\u932F\u8AA4"):r("\u597D\u50CF\u6709\u6771\u897F\u51FA\u932F\u4E86");else if(t&&t.list&&o&&e.choices){var u=e.choices.filter(e=>e.value==o.trim());0==u.length&&r(i.lang("Invalid answer. Clear & select a valid answer from the list"))}else i.data&&i.data.facade&&i.data.facade.items&&(e.overwrite=i.data.facade.items[e.id]),validateEngine(e,l,r)}),0<n.length&&(n[0].focus(),scrollIntoView(n[0])),n.length}function gotoSection(e={},t,i){var n,a=this.getDocument();if("back"==i)n=getPreviousSectionId(t),jumptoSection(e,t,i,n);else{this.saveDraft();var l=validate(e,t);if(0<l)return;n=i?i:getNextSectionId(t);var o=this.data?.scraped.items||{};a.querySelectorAll("#ff-sec-"+t+" .ff-nav-dyn").forEach(function(t={}){var i=[],a=t.id?t.id.split("-").pop():null,l=o[a]||{},r=e["entry."+l.entry]||{};l.choices&&(i=l.choices.filter(e=>e.value==r.value)),0<i.length&&(n=i[0].navigateTo)}),-1==n?n=t:-2==n?n=getNextSectionId(t):-3==n&&(n="ending")}jumptoSection(e,t,i,n)}function removeItemFromShoppingCart(e){var t=shoppingCart.indexOf(e);-1!==t&&shoppingCart.splice(t,1)}function updateItemFromShoppingCart(e){var t=shoppingCart.indexOf(e);-1===t?shoppingCart.push(e):shoppingCart[t]=e}function updateProduct(e,t,i){product_data[e].quantity=t?t:0,e&&(t?(draft.entry[e]=t,updateItemFromShoppingCart(e)):(delete draft.entry[e],removeItemFromShoppingCart(e))),calculateProduct(),renderBill(),i?(this.renderProduct(e,t),this.closePopup(e,t)):this.renderProduct(e,t)}function generateProductInnterHtml(e){for(var t,n="",a=1;10>a;a++)t="",t=draft.entry[e.product_id]&&draft.entry[e.product_id]==a?`<li class="col-qty-active" onclick="updateProduct(${e.product_id}, ${a}, true)" title="${a}" id="prdentry.${e.product_id}">
                     ${a}
                 </li>`:`<li onclick="updateProduct(${e.product_id}, ${a}, true)" title="${a}" id="prdentry.${e.product_id}">
            ${a} </li>`,n+=t;var l="";draft.entry[e.product_id]&&(l=`
    <div class="prdfooter">
			<a class="prddel" href="#!" onclick="updateProduct(${e.product_id}, null, true)">清空</a>
		</div>`);var o=`     
    <div class="prdheader">
	<div class="prdtitle">${e.title}</div>

        <div class="prdhelp">
                NT$${e.price}
        </div>
        <div class="prdinfo">	
            尺寸：${e.size}<br>
            材質：${e.material}
        </div>

    
	<div class="prdclose" onclick="closePopup(false)">
    <img src="./data/icon/close.svg" class="material-icons">

	</div>
</div>
	
<div>

<div class="prdwdg">
        <div class="col-qty">
        
            <!--<label>請選擇數量</label>-->
            
          <ul>
            ${n}
          </ul>
    </div>
</div>
${l}

</div>`;return o}function showProduct(e){this;this.product={id:e};var t=product_data[e];if(t&&"PARAGRAPH_TEXT"==t.type){var i=this.draft.entry?this.draft.entry[t.entry]:null;this.product.configurable=this.toConfigurable(i)}var n=document.getElementById("ff-addprd-overlay");n.classList.add("active");var a=document.getElementById("ff-addprd-popup");a.classList.add("active"),a.innerHTML=generateProductInnterHtml(t),document.body.style.overflowY="hidden"}function validateEngine(e,t,i){var n=this,a=t.get("entry."+e.entry);if(!e.validType&&e.overwrite&&e.overwrite.validation&&e.overwrite.validation.validType&&Object.assign(e,e.overwrite.validation),"CHECKBOX"==e.type){var l=t.getAll("entry."+e.entry),o=t.get("entry."+e.entry+".other_option_response"),r=!o||!o.trim(),d=e.validOperator,s=e.validValue;!1==isNaN(s)&&(s=parseInt(s));var u=e.validMessage;e.required&&0==l.length?i(n.lang("This is a required question")):e.required&&1==l.length&&"__other_option__"==l[0]&&r?i(n.lang("This is a required question")):"Atmost"==d&&l.length>s?(!u&&(u="Must select at most "+s+" options"),i(u)):"Atleast"==d&&l.length<s?(!u&&(u="Must select at least "+s+" options"),i(u)):"Exactly"==d&&l.length!=s&&(!u&&(u="Must select exactly "+s+" options"),i(u))}else if("MULTIPLE_CHOICE"==e.type){var o=t.get("entry."+e.entry+".other_option_response"),r=!o||!o.trim();e.required&&"__other_option__"==a&&r&&i(n.lang("This is a required question"))}else if("GRID"==e.type){if(e.required&&e.rows.forEach(function(e){var a=t.getAll("entry."+e.entry);0==a.length&&(u="This question requires one response per row",1==e.multiple&&(u="This question requires at least one response per row"),u=n.lang(u),i(u))}),e.onepercol){var c={};e.rows.forEach(function(e){t.getAll("entry."+e.entry).forEach(function(t){c[t]&&(u="Please don't select more than one response per column",u=n.lang(u),i(u)),c[t]=e.entry})})}}else if(e.overwrite&&"FILE_UPLOAD"==e.overwrite.type){var y=t.get("entry."+e.entry),u=e.validMessage;e.required&&!y&&(!u&&(u=n.lang("This is a required question")),i(u))}else if(a&&("TEXT"==e.type||"PARAGRAPH_TEXT"==e.type)){var f=e.validType,d=e.validOperator,u=e.validMessage;if(e.validDynamic&&e.validEntryId){var m=t.get("entry."+e.validEntryId);if("Number"==f){var g,p;!1==isNaN(m)&&(g=parseFloat(m)),!1==isNaN(a)&&(p=parseFloat(a)),isNaN(a)?b="Must be a number":!m||isNaN(m)?b="Comparison field must be a number":"GreaterThan"==d&&!1==p>g?b="Must be a number greater than "+g:"GreaterEqual"==d&&!1==p>=g?b="Must be a number greater than or equal to "+g:"LessThan"==d&&!1==p<g?b="Must be a number less than "+g:"LessEqual"==d&&!1==p<=g?b="Must be a number less than or equal to "+g:"EqualTo"==d&&p!=g?b="Must be a number equal to "+g:"NotEqualTo"==d&&p==g&&(b="Must be a number not equal to "+g),b&&i(u?u:b)}else if("Text"==f){var b,m=t.get("entry."+e.validEntryId);"EqualTo"==d&&a!=m?b="Must equal to "+e.validValue:"NotEqualTo"==d&&a==m&&(b="Must not equal to "+e.validValue),b&&i(u?u:b)}}else if("Number"==f){var b;e.validValue||(e.validValue=0);var p;!1==isNaN(a)&&(p=parseFloat(a));var s=e.validValue;!1==isNaN(s)&&(s=parseFloat(s)),isNaN(a)?b="Must be a number":"IsNumber"==d&&isNaN(a)?b="Must be a number":"WholeNumber"==d&&(isNaN(a)||0<=a.indexOf("."))?b="Must be a whole number":"GreaterThan"==d&&!1==p>s?b="Must be a number greater than "+s:"GreaterEqual"==d&&!1==p>=s?b="Must be a number greater than or equal to "+s:"LessThan"==d&&!1==p<s?b="Must be a number less than "+s:"LessEqual"==d&&!1==p<=s?b="Must be a number less than or equal to "+s:"EqualTo"==d&&p!=s?b="Must be a number equal to "+s:"NotEqualTo"==d&&p==s?b="Must be a number not equal to "+s:"Between"==d&&e.validValue2&&(p<s||p>parseFloat(e.validValue2))?b="Must be a number between "+e.validValue+" and "+e.validValue2:"NotBetween"==d&&e.validValue2&&p>=s&&p<=parseFloat(e.validValue2)&&(b="Must be a number less than "+e.validValue+" or greater than "+e.validValue2),b&&i(u?u:b)}else if("Text"==f){var b;"EqualTo"==d&&a!=e.validValue?b="Must equal to "+e.validValue:"NotEqualTo"==d&&a==e.validValue?b="Must not equal to "+e.validValue:"Contains"==d&&e.validValue&&!1==0<=a.indexOf(e.validValue)?b="Must contain "+e.validValue:"NotContains"==d&&e.validValue&&0<=a.indexOf(e.validValue)?b="Must not contain "+e.validValue:"Email"==d&&!1==/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,63}$/.test(a)?b="Must be an email":"URL"==d&&!1==/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/.test(a)&&(b="Must be a URL"),b&&i(u?u:b)}else if(e.validValue&&"Regex"==f){var b;a||(a="");var h=new RegExp(e.validValue,"g");if("Contains"==d&&!1==h.test(a))b="Must contain "+e.validValue;else if("NotContains"==d&&h.test(a))b="Must not contain "+e.validValue;else if("Matches"==d){var v=a.match(h),T=v&&1==v.length&&v[0]==a;T||(b="Must match "+e.validValue)}else if("NotMatches"==d&&a.match(h)){var v=a.match(h),T=v&&1==v.length&&v[0]==a;T&&(b="Must not match "+e.validValue)}b&&i(u?u:b)}else if("Length"==f){var b;e.validValue||(e.validValue=0),"MaxChar"==d&&a.length>parseInt(e.validValue)?b="Must be fewer than "+e.validValue+" characters":"MinChar"==d&&a.length<parseInt(e.validValue)&&(b="Must be at least "+e.validValue+" characters"),b&&i(u?u:b)}}}this.saveDraft=function(){},this.closePopup=function(e=!0){try{document.body.style.overflowY="auto";var t=document.getElementById("ff-addprd-overlay");t.classList.remove("active");var i=document.getElementById("ff-addprd-popup");i.classList.remove("active"),i.classList.remove("ff-consent-confirm")}catch{console.error("close pop error")}},this.renderProduct=function(e,t){var i=document.getElementById("ff-id-"+e);if(!t)i.lastElementChild.classList.contains("ff-sel-cart")&&i.removeChild(i.lastChild);else{var n=`<div onclick="showProduct(${e})" class="ff-sel-cart">
        <div class="ff-sel-cart-sm">x <b>${t}</b></div>
      </div>`;i.lastElementChild.classList.contains("ff-sel-cart")&&(i.removeChild(i.lastChild),i.lastElementChild=n),i.innerHTML+=n}};