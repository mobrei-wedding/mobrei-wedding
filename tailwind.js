...
function FormFacade(data) {
    ...
    this.addLinkTag = function (lnkurl) {
        ...
        var version = this.data.devEnv ? new Date().getTime() : 54;
...
<div class="ff-cta-center" style="margin-top:40px;">
    <h4>You can download Neartale app to track status and message us easily.</h4>
    ...
    this.showPayment = function (frm, secid) {
        ...
        if (document?.querySelector("form.ff-form")) {
            document?.querySelector("form.ff-form")?.scrollIntoView();
        }
        this.draft.pageHistory.push(secid);
        var doc = this.getDocument();
        var elms = doc.querySelectorAll('#ff-submit-' + secid + ' img');
    ...
    // unselected
    // formFacade.updateProduct(1230811956, this.title, true)
    // selected
    // formFacade.updateProduct(1230811956, null, true)
            ...
            fetch(`${baseurl}/consent/${userId}/${publishId}/${fc.formfillable.template}`, { method: "GET" }).then(function (result) {
                ...
                for (var itemId in scItems) {
                    ...
                    if (entryValue) {
                        ...
                        } else if (entryValue == '__other_option__') {
                            var entryValue = (curr.draft.entry || {})[`${sitem.entry}-other_option_response`];
                            data = data.replaceAll(placeholder, entryValue)
                        } else {
                            data = data.replaceAll(placeholder, entryValue)
                        }
                    } else {
                        data = data.replaceAll(placeholder, '')
                    }
                }
                curr.showConsentDialog(data);
            });
...
function CartSidebar() {
    ...
    this.fetch = function () {
        ...
        if (lis.length == 0 || formFacade.draft.submitted > 0) {
            var htm = '<li class="ff-cart-noitem"><div></div><div class="ff-cart-title">- Your cart is empty -</div></li>';
